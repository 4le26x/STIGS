<#
.SYNOPSIS
    Configures the system to use FIPS-compliant algorithms for encryption, hashing, and signing.
...
$regName = "Enabled"
$regValue = 1  # 1 = Enabled (Forces the system to use FIPS-compliant cryptography)
...
Try {
    ...
    Write-Host "Successfully enabled FIPS-compliant algorithms for encryption, hashing, and signing." -ForegroundColor Green
} Catch {
...
if ($verify -eq $regValue) {
    Write-Host "Verification: FIPS-compliant algorithms are successfully enabled." -ForegroundColor Green
} else {
